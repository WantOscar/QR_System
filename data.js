window.TIG_DATA={
  equipment:[
    ...
    {id:'EQ-010',sourceRow:'지역정보개발원-구성',maker:'퓨쳐시스템',model:'ITU',displayModel:'WFW410A0A0AHCA23060046',sn:'WFW410A0A0AHCA23060046',system:'Primary / Backup 이중화',status:'운영',received:'-',receivedPlace:'지역정보개발원 본원',org:'지역정보개발원',branch:'ITU 사용',site:'보안관리대역 IPSec',install:'-',issue:'IPSec VPN 이중화 구성',repair:'-',repairCount:0,project:'지역정보개발원 VPN 구성'},
...
// 제조사별 탐색에서는 넥스지만 제외하고, 고객사/업체별 데이터에는 전체 20대가 표시됩니다.
(function(){
  ...
  function patchLabels(){
    ...
    document.querySelectorAll('.navcard').forEach(card=>{
      ...
      if(title.textContent.trim()==='더케이예다함'){
        ...
        meta.innerHTML='<b>사용 장비 ITU · 제조사 퓨쳐시스템</b><br>'+meta.innerHTML;
        card.dataset.vendorPatched='1';
      }
    });

    // EQ-010은 제조사별 탐색에서 퓨쳐시스템 → ITU로 분류하되,
    // 장비 상세/공개 화면의 '모델' 표시는 사용자 요청값으로 표시합니다.
    const eq010=window.TIG_DATA.equipment.find(x=>x.id==='EQ-010');
    if(eq010 && eq010.displayModel){
      document.querySelectorAll('.k,.info-k').forEach(label=>{
        if(label.textContent.trim()==='모델' && label.nextElementSibling){
          label.nextElementSibling.textContent=eq010.displayModel;
        }
      });
    }
  }
